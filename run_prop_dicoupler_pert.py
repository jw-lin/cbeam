...
# mesh params #
core_res = 200                      # no. of line segments to use to resolve the core-cladding interface(s)
...
dicoupler = waveguide.Dicoupler(rcore,rcore+0.25,ncore,ncore+neps,dmax,dmin,nclad,coupling_length,a,core_res,core_mesh_size=core_mesh_size,clad_mesh_size=clad_mesh_size,split=False)
...
tag = "test_dicoupler_remesh2" # identifier for this computation
import propagator
...
mesh0,IORdict = dicoupler.make_intersection_mesh(0,dz0,adprop.mesh_dist_scale,adprop.mesh_dist_power,adprop.min_mesh_size,adprop.max_mesh_size,writeto=meshwriteto)
IORsq_diff_dict = dicoupler.IORsq_diff(IORdict)


zs,coupling_mats,neffs,vs = adprop.prop_setup_pert(0,z_ex,mesh0,IORdict,IORsq_diff_dict,tol,min_zstep,True,tag,max_zstep,True,min_degen_dif=1e-6,mode="remesh")
...
zs,coupling_mats,neffs,vs = adprop.prop_setup(0,z_ex,tol,save=True,tag=tag,degen_groups=degen_groups,min_zstep=2.5,max_zstep=320.,plot=False,mode="remesh",dz0=dz0)
